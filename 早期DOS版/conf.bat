...
::-----------------------------------------------------------------------游戏内容下载资地址-----------------------------------------------------------------------
set 模组版本=8
set 模组地址="https://link.jscdn.cn/sharepoint/aHR0cHM6Ly9nOTk2NDk1Ny1teS5zaGFyZXBvaW50LmNvbS86dTovZy9wZXJzb25hbC9nOTk2NDk1N19nOTk2NDk1N19vbm1pY3Jvc29mdF9jb20vRVM0QzlnV1FMS3RQaUFCVVNuemZlY0VCWTZPSnpwYjNteXhPeGYxRlRSZDhsdz9lPUNkOHFtMw.rar"
...
echo 下载启动器
cd %~dp0\tool
aria2 --split=12 --dir=%~dp0/ %启动器地址% -o BakaXL.exe
echo 下载完成
...
if not exist %exe% (
		cd %~dp0\tool
                                aria2 --split=12 --dir=%~dp0/data %模组地址% -o 模组-%模组版本%.rar
...
if not exist %exe% (
		cd %~dp0\tool
                                aria2 --split=12 --dir=%~dp0/data %材质地址% -o 材质-%材质版本%.rar
		echo %exe% 文件不存在，已下载该文件！
    ) else (
        echo %exe% 文件已存在，无需下载！
    )
...
if not exist %exe% (
		cd %~dp0\tool
                                aria2 --split=12 --dir=%~dp0/data %光影地址% -o 光影-%光影版本%.rar
		echo %exe% 文件不存在，已下载该文件！
    ) else (
        echo %exe% 文件已存在，无需下载！
    )
