...
namespace MC自动安装更新脚本2版.mods
{
    internal class 运行
    {
        ...
        public static void Aria2(string 下载地址)
        {
            ...
            string[] 指令 = { "--max-connection-per-server=16 --split=16 -k1M --user-agent=Chrome ", 下载地址 };
        ...
        {
            string 当前路径 = Directory.GetCurrentDirectory();
            string[] 指令 = { "--max-connection-per-server=16 --split=16 -k1M --user-agent=Chrome", " --dir=" + 目标位置, 下载地址 };
            string 调用aria2地址 = 当前路径 + "/Data/Tool/aria2.exe";
            TransferExe(调用aria2地址, 指令);
        ...
        {
            string 当前路径 = Directory.GetCurrentDirectory();
            string[] 指令 = { "--max-connection-per-server=16 --split=16 -k1M --user-agent=Chrome", " --dir=" + 目标位置, 下载地址, "-o " + 重命名 };
            string 调用aria2地址 = 当前路径 + "/Data/Tool/aria2.exe";
            TransferExe(调用aria2地址, 指令);
        ...
        public static void Curl(string 下载地址)
        {
            string[] 指令 = { "-o", 下载地址 };
            TransferExe("curl",指令);
        }
        ...
        {
            string 当前路径 = Directory.GetCurrentDirectory();
            string 调用tcping地址 = 当前路径 + "\\Data\\Tool\\tcping.exe";
            string[] 指令 = { 目标IP };
